<script lang="ts">
  ...
  import {deleteMessage} from './Storage.svelte'
...
{#each messages as message, i}
  {#if message.role === 'user'}
    <div class="message-container user">
      <div class="message-delete" on:click={deleteMessage(chatId, i)}>
        <div></div>
        <div></div>
      </div>
      <article
      ...
      >
        <div class="message-body content">
          ...
          <SvelteMarkdown source={message.content} options={markedownOptions}
    ...
    <div class="message-container">
      <article class="message is-success assistant-message">
        <div class="message-body content">
          ...
          {#if message.usage}
            <p class="is-size-7">
              This message was generated on <em>{message.model || defaultModel}</em> using <span
                    class="has-text-weight-bold">{message.usage.total_tokens}</span>
              tokens ~= <span
                    class="has-text-weight-bold">${getPrice(message.usage, message.model || defaultModel).toFixed(6)}</span>
            </p>
          {/if}
        </div>
      </article>
