...
const chatSettingsList: ChatSetting[] = [
  ...
  },
  {
    ...
    apiTransform: (chatId, setting, val: Record<string, number>) => {
      ...
      if (!val) return null
      const tokenized: Record<number, number> = Object.entries(val).reduce<Record<number, number>>((a, [k, v]) => {
...
const chatSettingLookup: Record<string, ChatSetting> = chatSettingsList.reduce<Record<string, ChatSetting>>((a, v) => {
...
const globalSettingLookup: Record<string, GlobalSetting> = globalSettingsList.reduce<Record<string, GlobalSetting>>((a, v) => {
