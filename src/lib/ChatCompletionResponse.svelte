...
import type {Message, Usage} from "./Types.svelte"
...
export class ChatCompletionResponse {
    ...
    public constructor() {
        ...
        this.finishPromise = new Promise<Message>((res, rej) => {
            this.finishResolver = res
        })
    }

    public updateFromSyncResponse(responseData: object) {
    ...
    public updateFromAsyncResponse(responseData: object) {
        this.message.model = responseData.model
