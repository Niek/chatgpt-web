...
export const chatRequest = async (
  ...
  const getNewWs = async (): Promise<WebSocket> => await new Promise<WebSocket>((resolve, reject) => {
    ...
    nws.onmessage = event => {
      ...
      inputPrompt = lastPrompt + (doLead && (lInputArray.length > 0) ? delimiter : '')
  ...
  if (stopSequencesC.length > 0) petalsRequest.extra_stop_sequences = stopSequencesC
  // Update token count
